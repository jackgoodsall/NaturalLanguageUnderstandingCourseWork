...
RUN_DIR = ROOT / 'experiments/runs/C_REMOTE_A40_019_transfer_seed3'
OUTPUT_ROOT = ROOT / 'outputs/deberta_v3_transfer_seed3'
...
for seed in SEEDS:
    ...
    seed_probs = logits_to_pos_probs(pred_output.predictions)
    seed_preds = threshold_predictions(seed_probs, 0.5)
    seed_metrics = metrics_dict(dev_labels, seed_preds)

    trainer.save_model(str(best_dir))
    TOKENIZER.save_pretrained(str(best_dir))
    trainer.state.save_to_json(str(run_dir / 'trainer_state_final.json'))
    np.save(run_dir / 'dev_probs.npy', seed_probs)
    ...
    seed_summaries.append({
        ...
        'metrics_at_threshold_0_5': seed_metrics,
        'train_metrics': train_output.metrics,
    })
    all_seed_probs.append(seed_probs)
    seed_model_dirs.append(str(best_dir))
...
ensemble_probs = np.mean(np.vstack(all_seed_probs), axis=0)
ensemble_preds = threshold_predictions(ensemble_probs, 0.5)
ensemble_metrics = metrics_dict(dev_labels, ensemble_preds)
threshold_probe = sweep_thresholds(dev_labels, ensemble_probs)

summary = {
    'run_id': 'C_REMOTE_A40_019',
    'variant': 'deberta_v3_transfer_seed3',
    'device': DEVICE,
    'config': CONFIG,
    'seed_runs': seed_summaries,
    'ensemble': {
        'model_dirs': seed_model_dirs,
        'metrics_at_threshold_0_5': ensemble_metrics,
...
pred_df = dev_df.copy()
pred_df['ensemble_positive_probability'] = ensemble_probs
pred_df['ensemble_pred_0_5'] = ensemble_preds.astype(int)
